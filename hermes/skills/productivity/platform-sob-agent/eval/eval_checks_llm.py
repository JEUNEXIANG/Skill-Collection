...
USER_TASK = """
Using the live sheet data above, audit each workflow step following the gates defined in your spec.
...
CHECKS:
  ...
  [✓] Required tabs exist in [Weekly Live View]
  ...
...
Continue for all steps: Pre-flight, Step 1, Step 2, Step 3a, Step 3b, Step 4, Step 5, Step 6, PC2 Step 1.
End with a single line: OVERALL: PASS or OVERALL: FAIL
"""


# ── Phase 2: Run LLM verifier ×N ───────────────────────────────────────────────
def run_llm_verifier(client, context_text, run_num):
    print(f"  Run {run_num:>2}/{NUM_RUNS}...", end=" ", flush=True)
...
def generate_report(results):
    ...
    print(f"\n  {'='*70}")
    print(f"  CONSISTENCY VARIANCE BY STEP (out of {n} runs)")
    print(f"  {'='*70}")
    print(f"  {'Step':<42} {'PASS':>5}  {'FAIL':>5}  {'Variance':<15}")
    print("  " + "\u2500" * 68)
    for step in all_steps:
        verdicts = [r["step_results"].get(step) for r in results if step in r["step_results"]]
        passes = verdicts.count("PASS")
        fails  = verdicts.count("FAIL")
        minority = n - max(passes, fails)
        variance = f"FLIPPED {minority}x" if minority > 0 else "stable"
        print(f"  {step:<42} {passes:>5}  {fails:>5}  {variance:<15}")
...
if __name__ == "__main__":
    ...
    if not api_key:
        print("ERROR: DEEPSEEK_API_KEY not set. Run: export DEEPSEEK_API_KEY=***")
        sys.exit(1)
    ...
    # Phase 2
    print(f"Phase 2: Running LLM verifier {NUM_RUNS} times (model={MODEL}, temp={TEMPERATURE})...")
    results = []
    for i in range(1, NUM_RUNS + 1):
        result = run_llm_verifier(client, context_text, i)
