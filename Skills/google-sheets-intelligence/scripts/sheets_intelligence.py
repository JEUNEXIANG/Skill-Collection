...
def parse_formula_dependencies(formula: str, current_sheet: str | None = None) -> list[str]:
    """Extract normalized cell/range dependencies from a formula.

    This keeps ranges as ranges instead of pretending they are two independent
    cell references. For example, SUM(A1:A10) returns A1:A10.
    """
    deps: list[str] = []
    occupied_spans: list[tuple[int, int]] = []

    for m in RANGE_RE.finditer(formula):
        sheet = (m.group("sheet") or current_sheet or "").strip("'")
        dep = m.group(0)
        if sheet and "!" not in dep:
            dep = f"{sheet}!{dep}"
        deps.append(dep)
    ...
    for m in CELL_REF_RE.finditer(formula):
        ...
        sheet = (m.group("sheet") or current_sheet or "").strip("'")
        ref = m.group("ref")
        deps.append(f"{sheet}!{ref}" if sheet else ref)
...
def split_a1_range(a1_range: str) -> tuple[str | None, str]:
    """Split Sheet!A1 syntax into sheet name and local range."""
    if "!" not in a1_range:
        return None, a1_range
    sheet, local = a1_range.rsplit("!", 1)
    return sheet.strip("'"), local


def scan_sections_from_rows(rows: list[list]) -> list[dict]:
    """Detect likely stacked table sections from row values."""
    sections = []
    section_keywords = ("Dashboard", "CNLS", "NPC", "Top", "Updated on")
    market_prefixes = ("MY ", "ID ", "TH ", "VN ", "PH ", "SG ", "BR ", "MX ")
    ...
    for idx, row in enumerate(rows):
        ...
        first = non_empty[0]
        has_keyword = any(k in " ".join(non_empty) for k in section_keywords)
        looks_like_market_section = first.startswith(market_prefixes)
        looks_like_header = len(non_empty) >= 3 and any(
            token.lower() in {"date", "seller", "metric", "gmv", "orders", "rate", "updated"}
            for token in (v.lower() for v in non_empty[:8])
        )

        if has_keyword or looks_like_market_section or looks_like_header:
            sections.append({
                "row": idx + 1,
                "first_cell": first,
...
def get_cell_snapshot(service, spreadsheet_id: str, cell_range: str) -> dict:
    """Read the current value/formula for a target range before updating."""
...
def analyze_sheet(service, spreadsheet_id: str, sheet_name: str | None = None) -> dict:
    ...
    for sheet_meta in sheets_meta:
        ...
        range_str = f"{quote_sheet_name(name)}!1:{sheet_info['row_count']}"
        try:
            resp = service.spreadsheets().values().get(
                spreadsheetId=spreadsheet_id,
                range=range_str,
            ...
            try:
                resp = service.spreadsheets().values().get(
                    spreadsheetId=spreadsheet_id,
                    range=(
                        f"{quote_sheet_name(name)}!A1:"
                        f"{col_to_a1(max(sheet_info['col_count'], 1) - 1)}{sheet_info['row_count']}"
                    ),
                    valueRenderOption="FORMULA",
                    majorDimension="ROWS",
                ).execute()
        ...
        for r in range(row_count):
            ...
            for c in range(max(max_cols, 1)):
                ...
                row_data[col_to_a1(c)] = {
                    ...
                    "references": parse_formula_dependencies(val, name) if is_formula else [],
                    "functions": parse_functions(val) if is_formula else [],
                }
                if is_formula:
                    sheet_info["formulas"][f"{name}!{cell_ref}"] = val
                    deps = parse_formula_dependencies(val, name)
        ...
        sheet_info["detected_sections"] = scan_sections_from_rows(rows)
...
def format_structure(result: dict) -> dict:
    ...
    for sheet in result["sheets"]:
        ...
        sheet_summary = {
            ...
            "detected_sections": sheet.get("detected_sections", [])[:12],
        }
...
def update_cell(
    ...
    *,
    ...
    before = get_cell_snapshot(service, spreadsheet_id, cell_range)
    if before["is_formula"] and not allow_formula_overwrite:
        return {
            "error": "target_contains_formula",
            "message": "Refusing to overwrite a formula cell without --allow-formula-overwrite.",
            "range": cell_range,
    ...
    if dry_run:
        return {
            ...
            "range": cell_range,
            "old_value": before["value"],
            "old_is_formula": before["is_formula"],
            "new_value": value,
            "new_is_formula": isinstance(value, str) and value.startswith("="),
        }
    body = {"values": [[value]]}
    ...
    *,
    ...
    if dry_run:
        return {
            ...
            "range": range_str,
            "new_values": values,
            "note": "Range dry-run does not expand every target cell; inspect dependencies separately before applying.",
        }
    body = {"values": values}
    result = service.spreadsheets().values().update(
        spreadsheetId=spreadsheet_id,
...
def batch_update(
    ...
    *,
    ...
    for cell_range, value in changes.items():
        ...
        if before["is_formula"] and not allow_formula_overwrite:
            preview["error"] = "target_contains_formula"
    ...
    if dry_run or errors:
        return {
            "dry_run": dry_run,
            "would_update": previews,
            "error": "formula_overwrite_blocked" if errors else None,
        }

    data = [{"range": cell_range, "values": [[value]]} for cell_range, value in changes.items()]
    result = service.spreadsheets().values().batchUpdate(
        spreadsheetId=spreadsheet_id,
        body={"valueInputOption": "USER_ENTERED", "data": data},
    ).execute()
    return {
        "updated_ranges": [item.get("updatedRange", "") for item in result.get("responses", [])],
...
def main():
    ...
    p_upd.add_argument("--dry-run", action="store_true", help="Show old/new values without writing")
    p_upd.add_argument(
        "--allow-formula-overwrite",
        action="store_true",
        help="Allow overwriting cells that currently contain formulas",
    )
    ...
    p_ur.add_argument("--dry-run", action="store_true", help="Show planned range update without writing")
    ...
    p_app.add_argument("--dry-run", action="store_true", help="Show rows to append without writing")
    ...
    p_batch.add_argument("--dry-run", action="store_true", help="Show old/new values without writing")
    ...
    # scan-sections
    p_sections = sub.add_parser("scan-sections", help="Detect likely stacked table sections")
    p_sections.add_argument("spreadsheet_id", help="Google Sheets ID")
    p_sections.add_argument("--sheet", "-s", help="Sheet name")
    ...
    if args.command == "analyze":
        ...
        result = update_cell(
            ...
            allow_formula_overwrite=args.allow_formula_overwrite,
        )
        print(json.dumps(result, indent=2, ensure_ascii=False))

    elif args.command == "update-range":
        values = json.loads(args.values)
        result = update_range(service, args.spreadsheet_id, args.range, values, dry_run=args.dry_run)
        print(json.dumps(result, indent=2, ensure_ascii=False))

    elif args.command == "append":
        values = json.loads(args.values)
        result = append_rows(service, args.spreadsheet_id, args.range, values, dry_run=args.dry_run)
        print(json.dumps(result, indent=2, ensure_ascii=False))
    ...
    elif args.command == "scan-sections":
        result = analyze_sheet(service, args.spreadsheet_id, args.sheet)
        sections = {
            sheet["name"]: sheet.get("detected_sections", [])
            for sheet in result["sheets"]
        }
