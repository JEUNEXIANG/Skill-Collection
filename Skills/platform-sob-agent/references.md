...
## Table Description Format: Crystallized, Not Coordinate-Based

Table descriptions should be organised around **semantic invariants** — what doesn't change — not fixed row/column numbers.

**Bad (coordinate-based):** "ADG in rows 2-39 cols C-K, ADO in rows 41-77 cols C-K"

**Good (crystallized):** describe what the table IS, how to recognise it dynamically (keywords, structural patterns, repeating column sequences), and what to exclude.

Key invariants to identify for any table:
...
- **Layout pattern:** stacked vertically, stacked horizontally (side-by-side same rows), or transposed (markets in rows)
