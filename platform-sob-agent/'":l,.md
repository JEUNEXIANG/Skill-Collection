...
3. Tab `SHP & TTS PC2` in `[Reg Commercial Team]`
...
3. Read tab `SHP & TTS PC2` in `[Reg Commercial Team]` using `$GSI scan-sections` to identify the reference values section (expected around rows `172:196` in the destination — verify by section headers).
...
1. From tab `SHP & TTS PC2` in `[Reg Commercial Team]`:
